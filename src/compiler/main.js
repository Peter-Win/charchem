...
const {ChainSys} = require('./chainSys')
const ChemAgent = require('../core/ChemAgent')
const ChemBond = require('../core/ChemBond')
const {ChemBrBegin, ChemBrEnd} = require('../core/ChemBr')
const ChemComment = require('../core/ChemComment')
const ChemCharge = require('../core/ChemCharge')
const ChemCustom = require('../core/ChemCustom')
const ChemError = require('../core/ChemError')
const ChemExpr = require('../core/ChemExpr')
const {ChemMul, ChemMulEnd} = require('../core/ChemMul')
const ChemNode = require('../core/ChemNode')
const ChemNodeItem = require('../core/ChemNodeItem')
const ChemRadical = require('../core/ChemRadical')
const ChemOp = require('../core/ChemOp')
const Lang = require('../Lang')
const Point = require('../math/Point')
const {preProcess} = require('../compiler/preprocess')
const {MenTbl, findElem} = require('../core/MenTbl')
const IsNonText = require('../visitors/IsNonText')
const {dashes, dots} = require('./utils')
const {ver, verStr} = require('../utils/version')

const _notImpl = fnName => {
...
const BondDefMap = {}	// Map 'first char'=>1 for parseNode optimization
 ...
 * @param {string} text formula description
 * @returns {ChemExpr} compiled expression
 */
const chemCompiler = text => {
	let
		result = new ChemExpr(),
		textLen,
	...
	/**
	 * set state
	 * @param {string} st       new state name
	 * @param {number=} res     result offset
	 * @returns {number}    result offset
	 */
	 ...
	 * @param {string} text with instructions
	 * @returns {string} text for display
	 */
	const cvtComm = text => {
		let p, res = text, kb, ke, s, a
		for (p of SpecChars) {
		...
		while (i < res.length) {
			...
			if (a) {
				...
				res = res.slice(0, kb) + a + res.slice(ke + 1)
			} else {
	...
	/**
	 * translate words in `Grave accents`
	 * @param {string} res source text with possible untranslated phrases. For ex: `He`
	...
	let specMass = 0, bAtomNum = 0, curColor = null, curItemColor = null, curItemColor1 = 0,
		...
		funcs = {
			...
			ver: args => {
				const formulaVer = args[0].split('.'), sysVer = ver()
				if (formulaVer.length > 1) {
					...
					if (formulaVer[0] > sysVer[0] || (formulaVer[0] == sysVer[0] && formulaVer[1] > sysVer[1]))
						error('Invalid version', {cur: verStr(), need:formulaVer.join('.')})
				}
			},
		}
	...
	function parseNode() {
		...
		case '"':	// Комментарий, который становится частью узла
			pos++
			{
				let item = makeComment(scanComment())
				pos++
				addNodeItem(item)
			}
			return setState(st_agentMid)
	...
	const isEmptyNode = node =>
		node.bAuto || ChemNode.isEmptyNode(node)


	const smartCreateNode = bAuto => {
	...
	function findNode(ref) {
		...
		const foundNode = nodesMap[ref]
		if (foundNode)
			return foundNode

		// если указан элемент (возможно с номером)
		const foundElement = _findElem(ref)
		if (foundElement) {
			const nodes = curEntity.nodes
			n = nodes.length
			for (i = 0; i < n; i++) {
				const possibleNode = nodes[i]
				if (possibleNode.items.length === 1 && possibleNode.items[0].obj === foundElement) {
					return possibleNode
				}
			}
		}
	...
	const findNodeEx = (refId, startPos) => {
		const node = findNode(refId)
